      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <svg class="w-5 h-5 text-green-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 0l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
          </svg>
          <span id="success-message" class="text-green-700 font-medium"></span>
        </div>

        <button id="copy-button"
          class="ml-4 px-3 py-1 bg-green-100 hover:bg-green-200 text-green-700 text-sm rounded-md transition-colors duration-200 flex items-center gap-1"
          style="display: none;">
    ...
    <form id="uploadForm" class="space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 dropzone rounded-xl hover:shadow-md transition">
              <div class="space-y-1 text-center">
                <svg class="mx-auto h-12 w-12 text-primary-500"
                  stroke="currentColor"
                  fill="none"
                  viewBox="0 0 48 48"
                  aria-hidden="true">
                ...
                <div class="flex text-sm text-gray-600 justify-center">
                  <label for="files"
                    class="relative cursor-pointer bg-white rounded-md font-medium text-primary-500 hover:text-primary-600 focus-within:outline-none transition">
            ...
            <div class="flex items-start">
                <div class="relative flex-grow">
                  ...
                  <input
                    ...
                    class="pl-[110px] block w-full px-4 py-3 border border-gray-300 rounded-2xl input-focus text-gray-900 transition">
                </div>

                <button
                  type="button"
                  class="ml-3 px-4 py-3 bg-white border border-gray-300 text-gray-700 rounded-2xl hover:bg-gray-50 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary-500 transition duration-300"
        ...
        <div>
            <div class="flex items-center space-x-2 mb-1">
                ...
                <label for="deleteDate" class="text-sm font-medium text-gray-700">
        ...
        <div>
            <div class="flex items-center space-x-2 mb-1">
                ...
                <label for="passwordProtected" class="text-sm font-medium text-gray-700">
            ...
            <div id="passwordContainer" class="hidden">
                ...
                <p class="text-xs text-gray-500 mt-1">
                  Users will need to enter this password to access the files.
                </p>
            </div>
        </div>
...
function renderUploadFormScripts(): string {
    ...
    // Use current domain instead of hardcoded rdrx.co
    const shortDomain = document.getElementById('shortDomain');
    if (shortDomain) {
      shortDomain.textContent = window.location.host;
    }
    ...
    const deleteAfterInput = document.querySelector('#deleteAfter');
    if (deleteAfterInput) {
      deleteAfterInput.value = defaultDate.toISOString().split('T')[0];
    }
    ...
    fileInput.addEventListener('change', () => {
        ...
        if (files.length > 0) {
            ...
            for (let i = 0; i < Math.min(files.length, 5); i++) {
                ...
                fileItem.className =
                  'flex items-center mt-2 p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition';

                fileItem.innerHTML = \`
                    <svg class="w-4 h-4 mr-2 text-primary-500"
                      fill="currentColor"
                      viewBox="0 0 20 20">
                        <path
                          fill-rule="evenodd"
    ...
    document.getElementById('uploadForm').addEventListener('submit', async (event) => {
        ...
        const passwordProtected =
          document.querySelector('#passwordProtected').checked;
        ...
        progressBar.className =
          'w-full bg-gray-200 rounded-full h-4 overflow-hidden';

        progressBar.innerHTML =
          '<div class="progress-gradient h-4 transition-all duration-200 ease-in-out" style="width: 0%"></div>';
        ...
        try {
            const response = await new Promise((resolve, reject) => {
                ...
                xhr.upload.addEventListener('progress', (e) => {
                    if (e.lengthComputable) {
                        ...
                        progressBar.firstChild.style.width =
                          percent + '%';
            ...
            // Build URL from the current website origin
            const shortUrl =
              window.location.origin + '/' + response.shortcode;

            successMessage.textContent =
              'Files uploaded successfully: ' + shortUrl;
    ...
    const deleteAfterCheck =
      document.querySelector('#deleteDate');

    const deleteAfter =
      document.querySelector('#deleteAfter');

    deleteAfterCheck.addEventListener('change', () => {
        deleteAfter.disabled =
          !deleteAfterCheck.checked;
    });
	`;
}
